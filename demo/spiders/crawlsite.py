...
class CrawlPens(CrawlSpider):
    ...
    if os.path.exists('./results.csv'):
        ...
        print('File deleted!!')
    if os.path.exists('./imageFiles.txt'):
        os.remove('./imageFiles.txt')
    ...
    def parse_items(self, response):
        ...
        item['insecureUrl']=response.css("script[src^='//']").getall()
        item['insecureStyles']=response.css("style[src^='//']").getall()
        f = open("imageFiles.txt", "a")
        for i in img_list:
            f.write(os.getenv('site') + i)
            f.write("\n")
        f.close()
